...
double newton(Fcn& f, Fcn& df, double x, int maxit, double tol, bool show_iterates){ // returns the root of the function using newton's method. Algorithm taken from the book.
	//check input
	if(maxit < 0){
	...
	fx = f(x);
		for(int i = 0; i <= maxit; i++){ //makes sure that the newton method doesn't run infinitely.
			...
			fx = f(x);
			if(show_iterates){
				cout << "Iterations: " << i << endl << "f(x)/f'(x) = " << d << endl << "new guess: " << x << endl;
			}
		}
		return x;
}
